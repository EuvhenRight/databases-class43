...
const createSqlDb = `CREATE DATABASE IF NOT EXISTS world`;
    ...
    // Select the database
    connectionDb.query('USE world', (err) => {
        if (err) throw err
...
const sqlWorld = fs.readFileSync(sqlPathFile, 'utf-8');
    ...
    sqlWorld.split(';').forEach((statement) => {
        connectionDb.query(statement, (err) => {
        if (err) throw err;
        });
    });
    ...
    connectionDb.query('SELECT Name FROM country WHERE Name LIKE "%land%"',
        (err, results) => {
            if (err) throw err
    ...
    connectionDb.query('SELECT Name FROM city WHERE CountryCode = "NLD"',
        (err, results) => {
            if (err) throw err
    ...
    connectionDb.query('SELECT Population FROM city WHERE Name = "Rotterdam"',
        (err, results) => {
            if (err) throw err
    ...
    connectionDb.query('SELECT SUM(Population) AS WorldPopulation FROM country',
        (err, results) => {
            if (err) throw err
